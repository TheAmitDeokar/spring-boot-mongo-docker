    ...
    // Deploy App as Docker Container in Docker Deployment server
     stage('Deploy App as Docker Container in Docker Deployment server'){
        sshagent(['ubuntudeploymentserver']) {
          ...
          sh "ssh -o StrictHostKeyChecking=no ubuntu@172.31.43.211 docker run -d --name mongo --network springbridgenetwork -e MONGO_INITDB_ROOT_USERNAME:root -e MONGO_INITDB_ROOT_PASSWORD:passw0rd mongo"

          sh "ssh -o StrictHostKeyChecking=no ubuntu@172.31.43.211 docker run -d --name springbootappcontainer --network springbridgenetwork -e MONGO_DB_HOSTNAME:mongo -e MONGO_DB_USERNAME:mongo -e MONGO_DB_PASSWORD:passw0rd -p 8082:8080 686255940829.dkr.ecr.ap-south-1.amazonaws.com/spring-boot-mongo-docker:${buildNumber}"
