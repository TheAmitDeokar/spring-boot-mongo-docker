    ...
    // Deploy App as Docker Container in Docker Deployment server
     stage('Deploy App as Docker Container in Docker Deployment server'){
        sshagent(['ubuntudeploymentserver']) {
          ...
          sh "ssh -o StrictHostKeyChecking=no ubuntu@172.31.43.211 docker run -d --name mongo -e MONGO_INITDB_ROOT_USERNAME:root --network springbridgenetwork -e MONGO_INITDB_ROOT_PASSWORD:passw0rd mongo"
